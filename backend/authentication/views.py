...
from .serializers import CookieTokenRefreshSerializer, CookieTokenObtainSerializer, \
    CustomEmailPassSerializer, RegisterSerializer, EmailActivationSerializer, EmailSendSerializer, \
    ResetPasswordSerializer, EmailSendSerializerWithCreation
...
class RegisterView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        tasks.send_activation_email.apply_async(kwargs=dict(to_email=serializer.validated_data['email']))
...
class EmailResendActivationView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(status=status.HTTP_200_OK, data={'status': 'activation email resent'})
...
class ConfirmResetPasswordView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        tasks.send_password_reset_email.apply_async(kwargs=dict(to_email=serializer.validated_data['email']))

        return Response(status=status.HTTP_200_OK, data={'status': 'reset password email sent'})
...
class LogoutView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        response.delete_cookie(key=settings.JWT_AUTH_REFRESH_COOKIE, path=settings.JWT_AUTH_REFRESH_COOKIE_PATH)
