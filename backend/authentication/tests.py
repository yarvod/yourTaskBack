...
class JWTEmailPassLoginTests(APITestCase):
    ...
    def test_login_wrong_credentials(self):
        ...
        resp = self.client.post(self.login_url,
                                {'email': 'wrongemail@phystech-job.ru', 'password': 'pa1ss'}, format='json')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        resp = self.client.post(self.login_url,
                                {'email': 'testuser@phystech-job.ru', 'password': 'wrongpwd'}, format='json')
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
        resp = self.client.post(self.login_url,
                                {'email': None, 'password': 'pa1ss'}, format='json')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_wrong_access_token(self):
        resp = self.client.post(self.login_url,
                                {'email': 'testuser@phystech-job.ru', 'password': 'pa1ss'}, format='json')
...
class TokenRefreshTests(APITestCase):
    ...
    def test_access_token_and_valid_refresh_token(self):
        ...
        self.assertNotEquals(self.refresh_token, response.cookies["refresh_token"].value)
...
class RegisterUser(APITestCase):
    ...
    def test_register(self, send_activation_email_mock):
        ...
        resp = self.client.post(self.activation_url, data=dict(uid=user.id, token=token))
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_resend_activation(self, send_activation_email_mock):
        ...
        send_activation_email_mock.apply_async.assert_called_with(kwargs=dict(to_email=self.data['email']))
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    @mock.patch('authentication.tasks.send_activation_email')
    def test_fail_resend_activation(self, send_activation_email_mock):
        resp = self.client.post(self.resend_url, data={'email': 'wrongemail@phystech-job.ru'})
        send_activation_email_mock.assert_not_called()
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
...
class ResetPasswordTests(APITestCase):
    ...
    def setUp(self) -> None:
        self.password_reset_confirm_url = reverse('api:authentication:password-reset-confirm')
    ...
    def test_success_change_password(self):
        resp = self.client.post(self.password_reset_confirm_url, data={'email': self.user.email})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        resp = self.client.post(self.password_reset_url,
                                data={'uid': self.user.id, 'token': self.token, 'password': 'newpasswordforuser',
                                      'password2': 'newpasswordforuser'})

        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    def test_incorrect_email(self):
        resp = self.client.post(self.password_reset_confirm_url, data={'email': 'wrongemail@phystech-job.ru'})
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_incorrect_token(self):
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        resp = self.client.post(self.password_reset_url,
                                data={'uid': self.user.id, 'token': 'incorrecttoken', 'password': 'newpasswordforuser',
                                      'password2': 'newpasswordforuser'})
    ...
    def test_incorrect_uid(self):
        ...
        resp = self.client.post(self.password_reset_url,
                                data={'uid': 250, 'token': self.token, 'password': 'newpasswordforuser',
                                      'password2': 'newpasswordforuser'})
    ...
    def test_password_mismatch(self):
        ...
        resp = self.client.post(self.password_reset_url,
                                data={'uid': self.user.id, 'token': self.token, 'password': 'newpasswordforuser2',
                                      'password2': 'newpasswordforuser'})
