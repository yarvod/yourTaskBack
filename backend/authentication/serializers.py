...
from authentication.services import gen_random_string, set_hash_with_ttl, get_user_hash, delete_hash
...
class CustomEmailPassSerializer(CookieTokenObtainSerializer):
    ...
    def validate(self, data):
        user = User.objects.filter(email=self.initial_data["email"],
...
class CookieTokenRefreshSerializer(TokenRefreshSerializer):
    ...
    def validate(self, attrs):
        ...
        if attrs['refresh']:
            ...
            try:
                old_token = jwt.decode(old_refresh, settings.SIMPLE_JWT['SIGNING_KEY'],
            ...
            if get_user_hash(user_id=old_token["user_id"]) == old_token["hash"]:
                ...
                ttl = int(settings.SIMPLE_JWT['REFRESH_TOKEN_LIFETIME'].total_seconds())
                set_hash_with_ttl(user_id=str(new_token["user_id"]), my_hash=new_token["hash"], ttl=ttl)
                return data
            else:
